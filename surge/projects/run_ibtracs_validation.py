...
description = (' Run ensemble of IBTRACS simulations for a project or list of projects. You can specify a number of run parameters. If you want to compare with observations, you must make sure obs.nc is in the project directory.')
...
parser.add_argument('-proj', type=str, help='Project name(s) / directory(s)', default = '.', nargs='+')
...
parser.add_argument('-fp_tg', type=str, help='Name of tidegauge file for validation in project directory, default=obs.nc', 
                    default = 'obs.nc')
parser.add_argument('-fp_val', type=str, help='Name of output validation file, default=validation.csv', 
...
parser.add_argument('-year_start', type=int, help='First year to use storms. default=2000', default=2000)
parser.add_argument('-year_end', type=int, help='Last year to use storms. default=2022', default=2022)
parser.add_argument('-ntilei', type=int, help='Number of parallel I tiles. default=1', default = 1)
parser.add_argument('-ntilej', type=int, help='Number of parallel J tiles. Default=1.', default = 1)
parser.add_argument('-cdmodel', type=str, help='Wind stress parameterization. Default=garratt77', default='garratt77')
parser.add_argument('-cdmax', type=float, help='Maximum value of stress Cd. Default=0.0035', default=0.0035)
parser.add_argument('-bstress', type=float, help='Bottom stress. Default=2e-3', default = 2e-3)
parser.add_argument('-scale', type=float, help='Wind Scaling. Default=1.', default = 1)
...
for pp, proj in enumerate( proj_list ):
    ...
    year_start = max( pd.to_datetime( ds_tg.time.min().values ).year + 1, year_start )
    year_end = min( pd.to_datetime( ds_tg.time.max().values ).year -1, year_end )
    ...
    for yy in year_list:
        ...
        if n_storms_year == 0:
            print(f'Year {yy} / {year_end} --> No tracks found', flush=True)
            continue
        ...
        for ii in range(n_storms_year):
            ...
            storm = tracks_yy[ii]
            print(f'Year {yy} / {year_end} --> {ii+1} / {n_storms_year} ---> {sid_yy[ii]}', flush=True)
            
            forcing.make_forcing( ds_grd, storm, args.cdmodel, 1, args.cdmax,
                                    './roms_frc.nc', args.scale )
            os.chdir('..')
            subprocess.run( f'python ./plot_forcing.py -proj {proj} -o plots/frc_{sid_yy[ii]}.png', shell=True,)
            os.chdir(proj)
